...
INSERT INTO FUNCIONARIO_ESPECIALIDAD (DOCUMENTO_FUNCIONARIO, ID_ESPECIALIDAD) VALUES (47442944, (SELECT MAX(ID)
                                                                                                 FROM ESPECIALIDAD));
-- CREACION DE TABLAS -> asociar a la otra tabla
CREATE TABLE PALABRAS_CLAVE_PROCEDIMIENTO (
  ID      NUMBER PRIMARY KEY,
  PALABRA VARCHAR2(50) UNIQUE NOT NULL
);

CREATE SEQUENCE palabras_proc_id_seq START WITH 1;

CREATE OR REPLACE TRIGGER palabras_proc_id
BEFORE INSERT ON PALABRAS_CLAVE_PROCEDIMIENTO
FOR EACH ROW

  BEGIN
    SELECT palabras_proc_id_seq.NEXTVAL
    INTO :new.id
    FROM dual;
  END;
-- PROCEDIMIENTO -> HACER DE NUEVO POR EL CAMBIO EN RESPUESTA
...
IS
  BEGIN
    DECLARE
      ...
      SELECT R.TEXTO
      FROM RESPUESTA R
        JOIN ESPECIALIDAD E ON R.ID_ESPECIALIDAD = E.ID
        JOIN TIPO_CONSULTA TC ON E.ID = TC.ID_ESPECIALIDAD
      ...
      WHERE TC.DESCRIPCION = tipo_de_consulta
            AND PC.PALABRA IN (SELECT PALABRA
                               FROM PALABRAS_CLAVE_PROCEDIMIENTO);
